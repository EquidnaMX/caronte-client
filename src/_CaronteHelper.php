...
class CaronteHelper
{
    ...
    /**
     * Handle a bad request response based on the context (API or Web).
     *
     * @param string $message The error message to be returned.
     * @param string|null $forward_url The URL to forward to in case of a web request. If null, it redirects back.
     * @return Response|RedirectResponse The response object based on the request context.
     */
    public static function badRequest(string $message, string $forward_url = null): Response|RedirectResponse
    {
        if (self::isAPI() || self::isHook()) {
